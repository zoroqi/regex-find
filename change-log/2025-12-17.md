...
- [x] 調整 generateExportCustom 替換邏輯
...
### 調整 `generateExportCustom` 替換邏輯

問題: 就有邏輯是用 `ReplaceAllString` 進行分組替換, 依次進行分組替換, 直到最後一個分組.
這個過程中有一個漏洞, 就是當靠前的分組中的匹配文本中包含了 `\$\d` 的內容, 就會導致之後的替換會進行過多的操作.
...
我期望的結果是 `a$1-b`, 但按照當前邏輯結果是 `ab-b`.

當前進行修正後的邏輯:

- 使用正則 `\$(\d+)` 將所有分組的序號按照順序存儲在數組中
- 將 `format` 中的所有 `\$(\d+)` 結構都替換成 `%s`
- 將匹配到分組按照序號數組拼接成一個字符串數組
- 使用 `fmt.Sprintf` 進行格式化

**額外發現的漏洞:**

在審查修正後的邏輯時, 發現了一個新的漏洞. 雖然新邏輯解決了分組替換順序的問題, 但它直接將用戶輸入的 `format` 字符串拼接後用於 `fmt.Sprintf`. 如果用戶的 `format` 字符串中包含 `%` 字符 (例如 `%s`, `%d`, 或 `100%`), `fmt.Sprintf` 會將其視為格式化指令. 這會導致兩個問題:
1.  如果 `format` 字符串中包含的格式化指令 (如 `%s`) 沒有對應的參數, 程序會發生 `runtime panic`.
2.  字符串中的 `%` 字符可能會被錯誤地解釋或移除, 而不是作為普通文本輸出.

**修復方案:**

在將 `$n` 佔位符替換為 `%s` 之前, 先對整個 `format` 字符串進行預處理, 將其中所有的 `%` 字符轉義為 `%%`. 這確保了 `fmt.Sprintf` 會將用戶輸入的所有 `%` 都當作普通字符處理, 從而修復了這個漏洞並增強了程序的健壯性.
...
| --- | --- | --- | --- |
| gemini-2.5-flash-lite | 5 | 20,593 | 528 |
| gemini-2.5-pro | 29 | 679,980 | 13,749 |
| gemini-2.5-flash | 3 | 33,160 | 1,485 |
